...
ggplot(merged) + stat_function(aes(x=Broad_Mean,color="Broad"),fun=dnorm) + stat_function(aes(x=UTSW_Mean,color="UTSW"),fun=dnorm,args=list(mean=mean(merged$UTSW_Mean,na.rm=TRUE),sd=sd(merged$UTSW_Mean,na.rm=TRUE))) + ylab("") + xlab("") + theme(legend.position = 'bottom',legend.text = element_text(color='black',face='bold'),legend.title = element_text(color='black',face='bold')) + labs(color='Group',y='') + scale_color_manual(values=c('blue','black')) + labs(title=paste0("Distribution of the Mean\nof All",filegroup," Feature Values"))
ggsave(paste0("MeanBellCurveAcross_",filegroup,"_Features.png"), type = "cairo")

ggplot(merged) + stat_function(aes(x=Broad_Standard_Deviation,color="Broad"),fun=dnorm) + stat_function(aes(x=UTSW_Standard_Deviation,color="UTSW"),fun=dnorm,args=list(mean=mean(merged$UTSW_Standard_Deviation,na.rm=TRUE),sd=sd(merged$UTSW_Standard_Deviation,na.rm=TRUE))) + ylab("") + xlab("") + theme(legend.position = 'bottom',legend.text = element_text(color='black',face='bold'),legend.title = element_text(color='black',face='bold')) + labs(color='Group',y='') + scale_color_manual(values=c('blue','black')) + labs(title=paste0("Distribution of the Standard Deviation\nof All",filegroup," Feature Values"))
