...
#add column that gives what replicate it is
datafile<-datafile %>% group_by(Metadata_broad_sample) %>% mutate(Replicate = cumsum(!duplicated(broad_well)))
#remove DMSO from df
datafile<-datafile[datafile$Metadata_broad_sample!="DMSO",]
...
df_new<-cbind(broad_replicate=datafile$broad_replicate,Metadata_Well=datafile$Metadata_Well,df_new)
df_new<-melt(df_new)
names(df_new)[names(df_new)=="variable"]<-"Measurement"
names(df_new)[names(df_new)=="value"]<-"Value"

#create a randomized dataframe
#randomize the value associated with the broad_replicate
df_new_rand<-data.frame(Metadata_Well=df_new$Metadata_Well,broad_replicate=sample(df_new$broad_replicate),Measurement=df_new$Measurement,Value=df_new$Value)
...
#cast the data frames so that each replicate has its own value and find the correlations. This is done for the normal dataframe and the randomized dataframe.
df_new_cast<-cast(df_new,Measurement+broad_sample~replicate,value="Value",fun.aggregate=sum)
...
df_new_cast$Measurement_broad_sample=paste0(df_new_cast$Measurement,"_",df_new_cast$broad_sample)

df_new_rand_cast<-cast(df_new_rand,Measurement+broad_sample~replicate,value="Value",fun.aggregate=sum)
...
setkey(df_new_cast,Measurement_broad_sample) #changing this from broad_sample to Measurement because I think that is what I need.
df_cor_test<-df_new_cast[,list(repA.B=cor(A,B, use="complete.obs"),repA.C=cor(A,C, use="complete.obs"),repA.D=cor(A,D, use="complete.obs"),repB.C=cor(B,C, use="complete.obs"),repB.D=cor(B,D, use="complete.obs"),repB.D=cor(B,D, use="complete.obs")),by=key(df_new_cast)]
