...
process createLoadDataCsvs{
    ...
    generate_load_data_broad_5chan.sh $imagedir
...
process illuminationMeasurement{
    ...
    file illumpipe from '/path/to/pipelines/Cell_Painting_Illum_8x_remade_illumgrant.cppipe'
    ...
    """
    cp \$(cat ${illumpipe}) .
    cellprofiler -c -r -p Cell_Painting_Illum_8x_remade_illumgrant.cppipe --data-file ${plateid}_load_data.csv -i $imagedir -o ${plateid}_out
...
process createIllumLoadDataCsvs{
    ...
    """
    echo ${plateid} > platename.txt
    generate_illumCsv_broad_5chan.sh ${plateid}_out ${plateid}_load_data.csv
    mv ${plateid}_out/load_data_with_illum.csv ${plateid}_out/${plateid}_load_data_with_illum.csv
    mv ${plateid}_out/${plateid}_load_data_with_illum.csv .
    echo ${plateid}_out > fakefile.txt
    """
